...
- Put the video in frontmatter, not in the prose. GitHub renders frontmatter as
  a small table at the top of the file, so it stays visible to GitHub readers,
  and the website renders a real player.
...
- Thumbnails are self-hosted (committed under `images/`), generated by
  ...
  time. They belong in module READMEs and landing pages — a unit's video goes in
  frontmatter instead.
